 ...
 * @author MOILE
 */
public class UpdateEmployeeStage extends JFXDialog{
    ...
    private JFXComboBox<String> e_department ;
    private JFXTextArea e_postalAddress, e_physicalAddress;
    private JFXTextField e_fname,e_omang, e_mname, e_lname, e_email;
    ...
    public UpdateEmployeeStage(EmployeeModel emp){
        this.employee = emp;
        ...
        if(employee != null){
            employeeProfilePic.setFill(new ImagePattern(new Image(SIMS.class.getResourceAsStream("/icons/male_user_100px.png"))));
        }
        
        JFXButton update = new JFXButton("Update", SIMS.getGraphics(MaterialDesignIcon.UPLOAD, "text-white", 18));
        update.getStyleClass().addAll("btn-xs", "btn-success");
        ...
        contentGrid.add(SIMS.setBorderContainer(getEmployeeDetails(), "Employee Details"), 0, 0);
        contentGrid.add(SIMS.setBorderContainer(getEmployeeContactDetails(), "Employee Contacts"), 0, 1);
        
        contentGrid.add(SIMS.setBorderContainer(getNextOfKinDetails(), "Next Of Kin Details"), 0, 2);
        contentGrid.add(SIMS.setBorderContainer(getNextOfKinContactDetails(), "Next Of Kin Contacts"), 0, 3);
        //----------------------------------------------------------------------
        ...
        if(employee != null){
            ...
            e_omang.setText(employee.getIdentity());
            e_employement.setValue(SIMS.getLocalDate(employee.getEnrollDate()));
            e_department.setValue(employee.getDepartment());
            ...
            e_dob.setValue(SIMS.getLocalDate(employee.getDob()));
    ...
    private void saveChanges() {
        if(employee != null){ //-- update --
            ...
            if(isValidateInputs()){
                ...
                employee.setIdentity(e_omang.getText().trim()); 
                ...
                employee.setDepartment(AdminQuery.getDepartmentByName(e_department.getValue()).getID());
                ...
                if(EmployeeQuery.updateEmployeeProfile(employee, kin, true)){
                    new DialogUI("Employee details has been updated successfully",
                                    DialogUI.SUCCESS_NOTIF,PARENT_STACK_PANE, this).show();
                    
                    EmployeeManagementView.employeeListWork.restart();
                }else{
                    
                    new DialogUI("An error encountered while trying to update Employee details",
                                    DialogUI.SUCCESS_NOTIF, root, this).show();
                }
            }else{
            ...
            if(isValidateInputs()){
                ...
                emp.setIdentity(e_omang.getText().trim()); 
                ...
                emp.setDepartment(AdminQuery.getDepartmentByName(e_department.getValue()).getID());
                ...
                if(EmployeeQuery.updateEmployeeProfile(emp, next, false)){
                    new DialogUI("Employee details has been added successfully",
                                    DialogUI.SUCCESS_NOTIF,PARENT_STACK_PANE, this).show();
                    close();
                    EmployeeManagementView.employeeListWork.restart();
                    
                }else{
                    new DialogUI("An error encountered while trying to add Employee details",
                                    DialogUI.SUCCESS_NOTIF, root, this).show();
                }
            }else{
                 new DialogUI("Please ensure that required fields are captured, before trying to add"
                                + " Employee information",
                                    DialogUI.ERROR_NOTIF, root, null).show();
    ...
    private GridPane getEmployeeDetails(){
        ...
        e_department = new JFXComboBox<>();
        e_department.setItems(AdminQuery.getDepartmentNames());
        e_department.setPromptText("Department");
        e_department.setLabelFloat(true);
        e_department.setPrefWidth(280);
        employeeDetails.add(e_department, 1, 3);
        
        e_omang = new JFXTextField();
        e_omang.setPromptText("Omang|Passport");
        e_omang.setLabelFloat(true);
        CCValidator.setFieldValidator(e_omang, "Required");
        e_omang.setPrefWidth(280);
        employeeDetails.add(e_omang, 0, 4);
        ...
        employeeDetails.add(gender, 0, 5, 2, 1);
