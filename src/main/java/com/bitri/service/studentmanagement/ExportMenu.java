...
import com.bitri.access.PopupButton;
...
public class ExportMenu extends JFXPopup{
    ...
    public ExportMenu(Node owner) {
        ...
        PopupButton pdf = new PopupButton("Portable Document Format(PDF)", SIMS.getIcon("PDF_96px.png", 24));
        ...
        PopupButton xls = new PopupButton("Excel Document (xls)", SIMS.getIcon("XLS_96px.png", 24));
        ...
        PopupButton txt = new PopupButton("Plain Text File (txt)", SIMS.getIcon("TXT_96px.png", 24));
        txt.setPrefWidth(240);
        txt.setOnAction((ActionEvent event) -> {
            hide();
        });
        
        container.getChildren().addAll(new HBox(SIMS.getIcon("FAQ_96px.png")), pdf, xls, txt);
    }
