...
public class UpdateStudentProfile extends JFXDialog{
    ...
    private JFXComboBox<String> s_batch, s_lastSchool, s_pslGrade;
    ...
    public UpdateStudentProfile(Student stdnt){
        ...
        if(student != null){
            studentProfilePic.setFill(new ImagePattern(new Image(SIMS.class.getResourceAsStream("/icons/male_user_100px.png"))));
        }
        
        JFXButton update = new JFXButton("Update", SIMS.getGraphics(MaterialDesignIcon.UPLOAD, "text-white", 18));
        update.getStyleClass().addAll("btn-xs","btn-success", "btn-block" );
        ...
        contentGrid.add(SIMS.setBorderContainer(getStudentDetails(), "Student Details"), 0, 0);
        contentGrid.add(SIMS.setBorderContainer(getStudentContactDetails(), "Student Contacts"), 0, 1);
        
        contentGrid.add(SIMS.setBorderContainer(getParentDetails(), "Parent/Guardian Details"), 0, 2);
        contentGrid.add(SIMS.setBorderContainer(getParentContactDetails(), "Parent/Guardian Contacts"), 0, 3);
        //----------------------------------------------------------------------
        ...
        if(student != null){
            ...
            s_batch.setValue(AdminQuery.getBatchByID(student.getBatchID()).getDescription());
            s_lastSchool.setValue(student.getLastSchoolAttended());
            s_pslGrade.setValue(student.getPslegrade());
            s_dob.setValue(SIMS.getLocalDate(student.getDob()));
            s_enrollmentDate.setValue(SIMS.getLocalDate(student.getEnrollDate()));
            s_lastSchool.setValue(student.getLastSchoolAttended());
            ...
            //------------------------------------------------------------------
            Guardian guardian = SIMS.dbHandler.getGaurdianByStudentID(student.getStudentID());
    ...
    private void saveChanges() {
        ...
        if(student != null){ //-- update --
            ...
            if(isValidateInputs()) {
                ...
                student.setBatchID((s_batch.getValue() != null) ? AdminQuery.getBatchByName(s_batch.getValue()).getId() : "");
                ...
                Guardian parent = SIMS.dbHandler.getGaurdianByStudentID(student.getStudentID());
                ...
                if (SIMS.dbHandler.updateStudentProfile(student, parent, true)) {
                    new DialogUI("Student details has been updated successfully",
                            DialogUI.SUCCESS_NOTIF, STUDENT_MAN_STACK, this).show();

                    StudentManagement.admin.studentEnrolment.slws.restart();
                } else {
                    new DialogUI("An error encountered while trying to update student details",
                            DialogUI.ERROR_NOTIF, root, null).show();
                }
            }else {
                    new DialogUI("Please ensure intake/ batch has been selected before saving changes",
                            DialogUI.ERROR_NOTIF, root, null).show();
                }
            }else{
            ...
            if(isValidateInputs()){
                ...
                std.setBatchID((s_batch.getValue() != null)? AdminQuery.getBatchByName(s_batch.getValue()).getId():"");
                ...
                if(SIMS.dbHandler.updateStudentProfile(std, parent, false)){
                    new DialogUI("Student enrolled successfully",
                                    DialogUI.SUCCESS_NOTIF, STUDENT_MAN_STACK, this).show();
                    close();
                    StudentManagement.admin.studentEnrolment.slws.restart();

                }else{
                    new DialogUI("An error encountered while trying to enroll student.",
                                    DialogUI.ERROR_NOTIF, root, null).show();
                }
            }else {
                new DialogUI("Please ensure intake/ batch has been selected before saving changes",
    ...
    private GridPane getStudentDetails(){
        ...
        s_batch = new JFXComboBox<>(AdminQuery.getBatchNames());
        s_batch.setPromptText("Batch");
        s_batch.setLabelFloat(true);
        s_batch.setPrefWidth(280);
        AutoCompleteComboBoxListener.setAutoCompleteValidator(s_batch);
        new AutoCompleteComboBoxListener(s_batch);
        studentDetails.add(s_batch, 1, 1);   
