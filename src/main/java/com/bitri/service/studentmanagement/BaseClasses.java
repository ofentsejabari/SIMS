...
import com.bitri.service.schooladministration.UpdateBaseClassDialog;
 ...
 * @author ofentse
 */
public class BaseClasses extends BorderPane{
    ...
    public BaseClasses() {
        ...
        JFXButton clear = new JFXButton("", SIMS.getGraphics(MaterialDesignIcon.CLOSE, "text-error", 20));
        ...
        JFXButton refresh = new JFXButton("Refresh", SIMS.getGraphics(MaterialDesignIcon.ROTATE_3D, "", 18));
        refresh.getStyleClass().addAll("btn-xs", "btn-default");
        refresh.setTooltip(new Tooltip("Refresh batch List"));
        ...
        JFXButton optionalSubjects = new JFXButton("Student Optional Subjects");
        optionalSubjects.setTooltip(new Tooltip("Optional subject student allocation"));
        optionalSubjects.getStyleClass().addAll("btn-xs", "btn-success");
        optionalSubjects.setGraphic(SIMS.getGraphics(OctIcon.DIFF, "text-white", 18));
        ...
        toolbar.getChildren().addAll(search, new HSpacer(), refresh, add, optionalSubjects,  btn_export);
                            ...
                            delete.setOnAction((ActionEvent event) -> {
                                ...
                                alert.showAndWait().ifPresent(response -> {
                                    if (response == ButtonType.OK) {
                                        ...
                                        if(StudentQuery.removeStudentFromBaseClass(ID)){
        ...
        table.getTableView().getColumns().addAll(admissionNumber, fname, contacts, ctrl);
    ...
    public class BatchClassListWork extends Task<ObservableList<HBox>> {       
        ...
        protected ObservableList<HBox> call() throws Exception {
            ...
            for (BaseClass bc: baseClass) {
                ...
                edit.getStyleClass().addAll("btn-default", "btn-xs");
                edit.setOnAction((ActionEvent event) -> {
                    new UpdateBaseClassDialog(AdminQuery.getBaseClassByID(bc.getClassID()));
                });
                ...
                JFXButton total = new JFXButton(""+ SIMS.dbHandler.getBaseClassStudentList(bc.getClassID()).size());
                ...
                HBox cn = new HBox(new Label(bc.getName()), new HSpacer(), view, edit,total);
    ...
    public class StudentClassListWork extends Task<ObservableList<Student>> {       
        ...
        protected ObservableList<Student> call() throws Exception {
            ...
            if(selectedClass != null){
                data = SIMS.dbHandler.getBaseClassStudentList(selectedClass.getClassID());
            }else{ 
