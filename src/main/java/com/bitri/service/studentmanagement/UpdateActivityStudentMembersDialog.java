 ...
 * @author jabari
 */
public class UpdateActivityStudentMembersDialog extends JFXDialog{

    public ExtraCurriculaActivity activity;
    private final SelectionListView view;
    private ObservableList<String> currentActivityStudentsID;
    private ObservableList<Student> allStudents;
    
    @SuppressWarnings("ResultOfObjectAllocationIgnored")
    public UpdateActivityStudentMembersDialog(ExtraCurriculaActivity activity, Service service) {
        
        //-- A list of student IDs in the current class --
        currentActivityStudentsID = FXCollections.observableArrayList();
        ...
        view.setTargetHeader(" Student(s)");
        ...
        save.setOnAction((ActionEvent event) -> {
            ...
            if(!view.getTarget().getItems().isEmpty() || !currentActivityStudentsID.isEmpty()){
                ...
                for(String sname: view.getTarget().getItems()){
                    ...
                    if(!currentActivityStudentsID.contains(sname)){ 
                ...
                for(String sID: currentActivityStudentsID){
    ...
    public void updateList(){
        ...
        allStudents = SIMS.dbHandler.getStudentList("");
        ...
        ObservableList<ActivityMember> current = AdminQuery.getExtraCurriculaActivitiesMembers(activity.getId(), "Student", false);
        
        for(ActivityMember std: current){
            target.add(std.getMemberID().trim());
            currentActivityStudentsID.add(std.getMemberID().trim());
        }
